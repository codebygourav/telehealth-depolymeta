...
import { Loader2, Mic, Search, Stethoscope, X } from "lucide-react";
...
export default function AddPrescriptionDialog({
  ...
  const [selectedSpeechLocale, setSelectedSpeechLocale] =
    useState<VoiceLocale>(() => getInitialVoiceLocale(assistantConfig?.speech_locale));
  ...
  useEffect(() => {
    ...
    resetDialogState({
      ...
      defaultSpeechLocale: getInitialVoiceLocale(assistantConfig?.speech_locale),
    });
  ...
  const startListening = () => {
    ...
    if (dictationMode === "guided") {
      transcriptBaseRef.current =
        (guidedTranscriptsRef.current[guidedStep] || "").trim();
    } else {
        ...
        <DialogContent className="w-[95vw] max-w-4xl! rounded-2xl p-0 overflow-hidden">
          <DialogHeader className="border-b px-4 py-4 sm:px-6">
            ...
            <p className="text-xs text-muted-foreground sm:text-sm">
              Choose voice or manual entry. Unknown medicine names will be saved
              as doctor-added medicines when you submit.
            </p>
          </DialogHeader>
          ...
          <div className="max-h-[82vh] overflow-y-auto px-4 py-4 sm:px-6 sm:py-5">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              <Tabs
                ...
                onValueChange={(value) => {
                  stopListening(false);
                  setEntryMode(value as EntryMode);
                }}
                className="gap-4"
              >
                <TabsList className="grid h-auto w-full grid-cols-2 rounded-xl bg-muted/60 p-1">
                  <TabsTrigger value="voice" className="py-2 text-sm">
                    Voice Prescription
                  </TabsTrigger>
                ...
                <TabsContent value="voice" className="space-y-4">
                  {!dictationEnabled ? (
                    <div className="rounded-xl border border-dashed bg-muted/20 p-5 text-sm text-muted-foreground">
                      Voice prescription is currently unavailable for this
                      account. Use the manual tab to add the prescription.
                    </div>
                  ) : (
                          ...
                          <div className="rounded-2xl border bg-background p-4">
                            ...
                            <div className="mt-4 flex flex-wrap items-center gap-2">
                              ...
                              <span className="text-xs text-muted-foreground">
                                Language: {voiceLanguageOptions.find((item) => item.value === selectedSpeechLocale)?.label}
                              </span>
                            </div>
                              ...
                              {guidedStep < guidedVoiceSteps.length ? (
                                <Button
                                  ...
                                  onClick={() => {
                                    stopListening(false);
                                    setGuidedStep((previous) =>
                                      Math.min(previous + 1, guidedVoiceSteps.length)
                                    );
                                  }}
                                  disabled={isListening}
                                >
                                ...
                                <Button
                                  ...
                                  disabled={
                                    ...
                                    !Object.values(guidedTranscripts).some((value) =>
                                      Boolean(value.trim())
                                    )
                                  }
                                >
                ...
                <TabsContent value="manual" className="space-y-4">
                  {(draftWarnings.length > 0 || draftMissingFields.length > 0) && (
                    <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
                      <p className="font-semibold">Review Prefilled Details</p>
                  ...
                  <div className="rounded-2xl border bg-background p-4 sm:p-5">
                    <div className="mb-4">
                      <p className="text-sm font-semibold">
                        Step {manualStep}: {manualSteps[manualStep - 1]?.title}
                      </p>
                      <p className="text-xs text-muted-foreground">
                          ...
                          {selectedMedicineName ? (
                            <div className="flex items-center justify-between rounded-xl border bg-muted/30 px-3 py-2">
                              <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                  {exactMedicineMatch?.source === "inventory"
                                    ? "Matched from medicine inventory"
                                    : exactMedicineMatch?.source === "doctor_added"
                              ...
                              {searchQuery.trim() !== "" && (
                                <div className="max-h-56 overflow-y-auto rounded-xl border bg-background">
                                  ...
                                  >
                                    <div>
                                      <p className="text-sm font-medium">
                                        Use custom medicine: &quot;{searchQuery.trim()}&quot;
                                      </p>
                                      <p className="text-xs text-muted-foreground">
                                        Save to doctor-added medicines if not found
                                      </p>
                                    </div>
                      ...
                      <div className="space-y-5">
                        ...
                        <div className="rounded-2xl border bg-muted/20 p-4">
                          ...
                          <div className="mt-3 grid gap-3 sm:grid-cols-2">
                            ...
                            <ReviewRow
                              label="Meal"
                              value={meal ? meal.replace("_", " ") : "Not selected"}
                            />
                            <ReviewRow
                              label="Start Date"
                    ...
                    <div className="mt-6 flex items-center justify-between gap-3">
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          setManualStep((previous) => Math.max(previous - 1, 1))
                        }
                      ...
                      {manualStep < manualSteps.length ? (
                        <Button type="button" onClick={handleNextManualStep}>
                          Next
                        </Button>
                      ) : (
...
function resetDialogState({
  ...
  setEntryMode(dictationEnabled ? "voice" : "manual");
  setDictationMode("guided");
  setManualStep(1);
  setGuidedStep(1);
