...
const MedicineAccordionItem = ({ medicine, index }: { medicine: Medicine; index: number }) => {
                ...
                >
                        ...
                        <div className="mt-3 sm:mt-4 p-2 sm:p-3 bg-blue-50 rounded-lg">
                            ...
                            <ul className="list-disc list-inside space-y-0.5 sm:space-y-1">
                                {medicine.instructions.map((ins: string, i: number) => (
                                    <li key={`${medicine.prescription_id}-instruction-${i}`} className="text-[11px] sm:text-sm text-blue-800 wrap-break-word">{ins}</li>
                                ))}
                            </ul>
                        </div>
                    )}
                    ...
                    {medicine.notes && (
                        <div className="mt-3 p-2 sm:p-3 bg-yellow-50 rounded-lg">
                            ...
                            <p className="text-[11px] sm:text-sm text-yellow-800 wrap-break-word">{medicine.notes}</p>
                        </div>
                    )}
                </CardContent>
            )}
...
export default function PrescriptionTab({ appointmentId }: { appointmentId: string }) {
    ...
    // Conclusion data
    const conclusionType = conclusionData?.data?.conclusion_report_files?.map((file: any) => file.type) || [];
    console.log("conclusionType", conclusionType);
    const fileUrl = conclusionData?.data?.conclusion_report_files?.map((file: any) => file.file_url) || [];
    console.log("fileUrl", fileUrl);

    // Check if both prescription and conclusion are empty
    const hasPrescriptionData = medicines.length > 0 || instructionsByDoctor || nextVisitDate || pdfUrl;
    const hasConclusionData = conclusionType.length > 0 || fileUrl.length > 0 || instructionsByDoctor || nextVisitDate;
                ...
                <Card>
                    <CardContent className="p-6 sm:p-8 text-center">
                        <div className="flex flex-col items-center gap-2 sm:gap-3">
                            ...
                            <p className="text-xs sm:text-sm text-muted-foreground">No prescription or conclusion available</p>
                            <p className="text-xs sm:text-sm text-muted-foreground">Start by adding a prescription or conclusion</p>
                        </div>
                    </CardContent>
                </Card>
        ...
        <div className="space-y-4 sm:space-y-5 md:space-y-6">
            ...
            {conclusionType.includes("other") && fileUrl.length > 0 && (
                <div className="mt-3 sm:mt-4 p-4 border rounded-lg">
                    <p className="text-base font-medium tracking-wide mb-1 sm:mb-2">Uploaded by Doctor</p>
                        ...
                        {fileUrl.map((url: string, index: number) => (
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm font-medium text-blue-800 truncate flex-1">File {index + 1}</span>
                                </div>
