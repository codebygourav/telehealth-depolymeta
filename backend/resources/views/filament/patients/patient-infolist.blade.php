@php
    ...
    $userStatus = User::find($patient->user_id)->status ?? null;
    ...
    $weekAppointments = $patient->appointments()
        ->whereIn('status', [
    ...
    $firstUpcomingDay = $weekDays
        ->first(fn ($dayDate) => $dayDate > $todayDate && (($appointmentsByDay->get($dayDate)?->isNotEmpty()) ?? false));
    $firstUpcomingDay = $firstUpcomingDay
        ?? $weekDays->first(fn ($dayDate) => $dayDate > $todayDate)
        ?? '__no_upcoming__';
    ...
    $appointmentUrl = function ($appointment): string {
        return \App\Filament\Resources\Appointments\AppointmentResource::getUrl(
            'view',
    ...
    $previousAppointment = $weekAppointments
        ->last(fn ($a) => $a->appointment_date && $a->appointment_date->toDateString() <= $todayDate);
    $nextAppointment = $weekAppointments
        ->first(fn ($a) => $a->appointment_date && $a->appointment_date->toDateString() >= $todayDate);
    ...
    $emailVerifiedAt = User::find($patient->user_id)->email_verified_at;
    ...
    $patientFillableKeys = (new Patient())->getFillable();
    $patientRows = [];
    ...
    $personalPatientKeys = array_values(array_unique([
        'first_name',
        'last_name',
        'age',
        'email',
    ...
    $locationPatientKeys = array_values(array_unique([
        'address',
        'pincode',
        'area',
        'city',
    ...
    $medicalPatientKeys = array_values(array_unique([
        'allergies',
        'existing_conditions',
        'current_medications',
    ...
    $emergencyInsurancePatientKeys = array_values(array_unique([
    ...
    $usedPatientKeys = array_values(array_unique(array_merge(
        $personalPatientKeys,
        $locationPatientKeys,
        $medicalPatientKeys,
    ...
    $accountRows = [
        ...
        'User Status Value' => User::find($patient->user_id)->status,
...
<x-dynamic-component :component="$getEntryWrapperView()" :entry="$entry">
    <div class="flex flex-col gap-6 pb-8">
        {{-- 1. HERO PROFILE --}}
        <div class="relative bg-white dark:bg-gray-900 rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-800">
            ...
            <div class="relative pt-12 px-6 sm:px-8 pb-8 flex flex-col md:flex-row gap-6 items-start md:items-center">
                <div class="shrink-0 relative">
                    <div class="w-28 h-28 rounded-full border-4 border-white dark:border-gray-900 overflow-hidden bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
                        @if (!empty($patient->avatar))
                            <img
                                src="{{ storage_url($patient->avatar) }}"
                                alt="{{ $fullName }}"
                ...
                <div class="flex-1 space-y-3">
                    <div>
                        ...
                        <div class="flex flex-wrap items-center gap-2 mt-2">
                            ...
                            >
                                <x-dynamic-component
                                    :component="$canLogin ? 'heroicon-o-check-circle' : 'heroicon-o-x-circle'"
                                    class="w-4 h-4"
                                />
                            ...
                            <span class="text-xs font-bold px-3 py-1 bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-md">
                        ...
                        <div class="flex flex-wrap items-center gap-3 mt-3 text-sm text-gray-600 dark:text-gray-300">
                            @if (!empty($patient->date_of_birth))
                                <span class="flex items-center gap-1.5 px-3 py-1 bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-full font-medium">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center gap-2 w-full md:w-auto">
                        @if ($phone)
                            <a href="tel:{{ $phone }}" class="flex items-center gap-2 px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                                <x-heroicon-o-phone class="w-4 h-4 text-primary" />
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-200">{{ $phone }}</span>
                            </a>
                        @endif
                        @if ($email)
                            <a href="mailto:{{ $email }}" class="flex items-center gap-2 px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition text-wrap break-all">
                                <x-heroicon-o-envelope class="w-4 h-4 text-primary" />
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-200 truncate max-w-[200px]">{{ $email }}</span>
                            </a>
                        @endif
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-2 w-full md:w-auto">
                    ...
                    <form method="POST" action="{{ route('admin.patients.notify-next', $patient) }}">
                        ...
                        <button
                            type="submit"
                            class="px-4 py-2 rounded-lg bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-600/90 transition"
                        >
            ...
            <div class="mt-5 flex flex-col lg:flex-row gap-6 items-start">
                <div class="w-full lg:w-1/3 flex flex-col gap-4">
                    <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5">
                        ...
                        <div class="space-y-3">
                            @foreach ($personalPatientKeys as $key)
                                @php
                                    $value = $patientFieldValuesByKey[$key] ?? null;
                                    $label = $labelForKey($key);
                                @endphp
                                <div class="flex items-start justify-between gap-4">
                                    <span class="text-xs font-medium text-gray-500 whitespace-nowrap pr-2">{{ $label }}</span>
                    ...
                    <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex items-start justify-between gap-4">
                                <span class="text-xs font-medium text-gray-500 whitespace-nowrap pr-2">Registration Status</span>
                                <span class="text-sm font-semibold text-gray-900 text-right break-words">
                                    {{ $registrationStatusLabel }}
                                </span>
                            </div>
                            <div class="flex items-start justify-between gap-4">
                                <span class="text-xs font-medium text-gray-500 whitespace-nowrap pr-2">Email Verified</span>
                                <span class="text-sm font-semibold text-gray-900 text-right break-words">
                        ...
                        <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                @foreach ($otherPatientKeys as $key)
                                    ...
                                    @endphp
                                    <div class="flex items-start justify-between gap-4">
                                        <span class="text-xs font-medium text-gray-500 whitespace-nowrap pr-2">{{ $labelForKey($key) }}</span>
                                        <span class="text-sm font-semibold text-gray-900 text-right break-words">
                                            {{ $formatFieldValue($value) }}
                                        </span>
                                    </div>
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6">
            <div class="flex items-center justify-between gap-4 flex-wrap">
                <h2 class="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    ...
                    Appointments ({{ $selectedWeekIncludesToday ? 'Current Week' : 'Selected Week (' . $weekStart->format('d M') . ' - ' . $weekEnd->format('d M') . ')' }})
                </h2>
                <div class="flex items-center gap-2">
                    @php
                        $prevWeekStart = $weekStart->copy()->subWeek()->toDateString();
                        $nextWeekStart = $weekStart->copy()->addWeek()->toDateString();
                        $prevWeekUrl = request()->fullUrlWithQuery(array_merge(request()->query(), ['week_start' => $prevWeekStart]));
                        $nextWeekUrl = request()->fullUrlWithQuery(array_merge(request()->query(), ['week_start' => $nextWeekStart]));
                    @endphp
            ...
            <div class="mt-5" x-data="{ tab: 'current', activeDay: '{{ $currentDayInWeek }}' }">
                ...
                <div class="flex items-center gap-2 flex-wrap mb-4">
                    <button
                        type="button"
                        @click="tab = 'current'; activeDay = '{{ $currentDayInWeek }}'"
                    ...
                    <button
                        type="button"
                        @click="tab = 'upcoming'; activeDay = '{{ $firstUpcomingDay }}'"
                ...
                <div
                    x-show="tab === 'upcoming' && activeDay === '__no_upcoming__'"
                    ...
                    @foreach ($weekDays as $dayDate)
                        ...
                        <div x-show="activeDay === '{{ $dayDate }}'" x-cloak>
                            @if ($dayAppointments->isEmpty())
                                <div class="text-sm text-gray-500 bg-gray-50/50 border border-gray-100 rounded-xl p-4 text-center">
                                ...
                                <div class="grid grid-cols-1 gap-3">
                                    @foreach ($dayAppointments as $appointment)
                                        @php
                                            ...
                                            $reason = $normalizeVisitReason($appointment->visit_reason ?? $appointment->notes ?? null);
                                            $reasonText = is_array($reason) ? implode(', ', array_filter($reason)) : $reason;

                                            [$paymentLabel, $paymentId, $paymentMethod] = $appointmentPaymentId($appointment);
                                        ...
                                        <div class="p-4 rounded-xl border border-gray-100 hover:border-primary/30 hover:bg-primary-50/30 transition">
                                            <div class="flex items-start justify-between gap-3">
                                                <div class="flex items-start gap-3">
                                                    <span class="inline-flex items-center px-3 py-1 rounded-xl text-xs font-semibold border {{ $statusColors[$statusValue] ?? 'bg-gray-100 text-gray-800 border-gray-200' }}">
                                                        <x-dynamic-component :component="match($statusValue){
                                                            ...
                                                            AppointmentStatus::RESCHEDULED->value => 'heroicon-o-arrow-path',
                                                            AppointmentStatus::CANCELLED->value, AppointmentStatus::FAILED->value, AppointmentStatus::NO_SHOW->value => 'heroicon-o-x-circle',
                                            ...
                                            <div class="mt-3 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                                                <div class="text-sm text-gray-700 dark:text-gray-200">
                                                    ...
                                                    <div class="text-gray-600">{{ $consultationTypeLabel($appointment->consultation_type) }}</div>
                                                </div>
