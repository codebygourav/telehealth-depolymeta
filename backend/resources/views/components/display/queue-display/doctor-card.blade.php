...
<div {{ $attributes->class(['doctor-card-shell']) }}
    @if($alpine)
        x-show="currentDoctor()"
        x-cloak
    @endif
>
                 ...
                 x-init="
                     ...
                     setInterval(() => {
                         if ($el.scrollHeight > $el.clientHeight) {
                             let row = $el.querySelector('.queue-row');
                             let step = row ? row.offsetHeight + 16 : 110;
                ...
                @if($alpine)
                    <template x-if="currentDoctor() && currentDoctor().queue_items && currentDoctor().queue_items.length">
                        <template x-for="item in currentDoctor().queue_items" :key="item.token + item.patient">
                            <div class="queue-row fade-enter" :class="{ 'active': item.is_active, 'next': item.turn === 'Next' || item.turn === 'You\'re Next' }">
                                ...
                                <div>
                                    <div class="status-pill" :class="item.is_active ? 'active-pill' : (item.turn === 'Next' || item.turn === 'You\'re Next' ? 'next-pill' : 'waiting')" x-text="item.status_label || item.status"></div>
                                </div>
                                <div class="turn" x-text="(item.turn === 'Next' || item.turn === 'You\'re Next') ? 'You\'re Next >>' : item.turn"></div>
                            </div>
                        </template>
                    </template>
                    ...
                    <template x-if="!currentDoctor() || !currentDoctor().queue_items || !currentDoctor().queue_items.length">
                        <div class="queue-row" style="grid-template-columns: 1fr;">
                            <div>
                                <div class="patient" x-text="displayCopy.empty_state_title || 'No active doctor assigned'"></div>
                                <div class="phone" x-text="displayCopy.empty_state_text || 'Please wait while we load the queue.'"></div>
                            </div>
                        </div>
                    </template>
                @else
                    @forelse($doctorQueueItems as $item)
                        <div class="queue-row fade-enter {{ !empty($item['is_active']) ? 'active' : '' }} {{ ($item['turn'] ?? '') === 'Next' || ($item['turn'] ?? '') === 'You\'re Next' ? 'next' : '' }}">
                            ...
                            <div>
                                <div class="status-pill {{ !empty($item['is_active']) ? 'active-pill' : ((($item['turn'] ?? '') === 'Next' || ($item['turn'] ?? '') === 'You\'re Next') ? 'next-pill' : 'waiting') }}">
                                    {{ $item['status_label'] ?? ($item['status'] ?? '') }}
                                </div>
                            </div>
                            <div class="turn">
                                @if(($item['turn'] ?? '') === 'Next' || ($item['turn'] ?? '') === 'You\'re Next')
                                    You're Next
                                @else
                                    {{ $item['turn'] ?? '' }}
                                @endif
    ...
    <div class="doctor-card-bg-icon" @if($alpine) x-cloak @endif>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round" style="width: 100%; height: 100%;">
