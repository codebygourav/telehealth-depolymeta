...
@props([
    ...
    'secondary' => false,
    // custom prop to allow overriding inner content classes (e.g. '!p-0')
    'contentClass' => null,
])
...
>
    ...
    @if ((! is_slot_empty($slot)) || (! is_slot_empty($footer)))
        ...
        >
            @if ($hasContentEl)
                {{-- Apply custom inner classes via the `content-class` / `contentClass` prop --}}
                <div class="fi-section-content {{ $contentClass ?? '' }}">
                    {{ $slot }}
                </div>
            @else
                {{ $slot }}
